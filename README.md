...
- 今天、可选收集箱、项目、备注、截止时间与完成状态，断网时照常编辑；已完成任务下沉保留并显示删除线，可随时恢复。
...
- iOS 内置原生 RSS 阅读器，支持来源分类、未读/收藏、搜索、OPML、离线缓存、关键词提醒、阅读进度与系统中英翻译；文章不会强制跳到外部浏览器。
- Android 与 Windows 使用 Flutter 客户端，四端共用同一份同步数据与协议。
- Windows 默认是置顶计时胶囊，悬停后在下方展开今日任务；设置、隐藏和退出集中在系统托盘。
- 全局采用纸白、石墨与炭黑的高级单色体系，只用明度和留白区分层级；可跟随系统或手动切换。
...
- RSS 同步保存订阅、分类、已读、收藏和阅读进度；文章正文只缓存在本机，既减少服务端占用，也避免把整篇内容上传到同步服务器。
- 同一任务按 `updatedAt` 合并，删除墓碑优先，避免旧设备复活已删除任务。
- 番茄钟以共享的 `endsAt` 和服务端时间计算剩余秒数；开始、暂停、重置、跳过和阶段完成会立即推送，并通过 `/v1/changes` 通知其他前台设备。iOS 在本机启动后由 Live Activity 在锁屏和灵动岛继续倒计时，退到桌面不依赖 App 每秒运行。无变化时请求在服务端休眠，不执行高频完整同步；5 分钟完整同步只用于容错兜底。
