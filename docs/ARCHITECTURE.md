...
主题设置属于设备偏好，不参与同步。任务、项目字段、备注、日期、完成/删除状态、番茄钟状态及 RSS 订阅阅读状态属于用户数据，会自动同步；RSS 文章正文仅保存在客户端缓存。
...
3. 前台保持一个低占用长轮询等待修订号变化，变化后才拉取完整状态；5 分钟同步作为断线兜底。
4. 任务按 ID 和更新时间合并，删除墓碑优先。
5. 番茄钟和 RSS 同步状态是单例 LWW 文档；番茄钟运行状态保存绝对 `endsAt`，客户端使用 `serverTime` 校准后的时钟计算剩余值。
...
- `POST /v1/sync`：合并并返回完整任务集、番茄钟状态、RSS 阅读状态和服务端时间。
