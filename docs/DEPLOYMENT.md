...
客户端保持一个最长 25 秒的低成本变更等待请求。任意设备修改后，服务端只广播新的修订号，其他前台设备再拉取完整数据；没有变化时不会反复上传任务。番茄钟操作立即推送，普通任务继续防抖合并上传，5 分钟完整同步仅作断线容错。设备时钟存在小幅偏差时，客户端使用响应中的 `serverTime` 校准同一个 `endsAt` 倒计时。
